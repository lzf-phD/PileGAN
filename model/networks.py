...
def define_G(input_nc, output_nc, input_txt_nc, output_txt_nc, ngf, netG, 
    ...
    if netG == 'global':
        netG = GlobalGenerator1(input_nc, output_nc, input_txt_nc, output_txt_nc, ngf,
...
class TXT_GLoss(nn.Module):
    def __init__(self):
        ...
        self.criterion_cossim = nn.CosineSimilarity(dim=0, eps=1e-6) #计算余弦相似度
...
class TXT_DLoss(nn.Module):
    ...
    def forward(self, img, txt, label):
        ...
        for i in range(len(img)): #只有1张img
...
class LocalEnhancer(nn.Module):
    def __init__(self, input_nc, output_nc, input_txt_nc, output_txt_nc, ngf=32, n_downsample_global=3, n_blocks_global=9, 
        ...
        model_global = GlobalGenerator1(input_nc, output_nc, input_txt_nc, output_txt_nc, ngf_global, n_downsample_global, n_blocks_global, norm_layer).model
...
#PileFAN1
class GlobalGenerator1(nn.Module): #netG的神经网络模型，nn.Module继承了pytorch的类
    def __init__(self, input_nc, output_nc, input_txt_nc, output_txt_nc, ngf=64, 
        ...
        activation = nn.ReLU(True)  #激活全部采用relu
        ...
        ## txt upsample
        txt_gen1 = [nn.ReflectionPad2d(3), nn.Conv2d(4, ngf, kernel_size=7, padding=0), norm_layer(ngf),activation]
        for i in range(n_downsampling):
              mult = 2 ** i
              txt_gen1 += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1),
                             norm_layer(ngf * mult * 2), activation]
        self.model_gen_txt = nn.Sequential(*txt_gen1)
        ...
        ### upsample         
        for i in range(n_downsampling): #0-3
    ...
    def forward(self, input_img, input_txt):
        input_img1 = self.model_gen_img(input_img)
        input_txt1 = self.model_gen_txt(input_txt)
        input_gen1 = input_img1*input_txt1
...
class GlobalGenerator2(nn.Module):
    def __init__(self, input_nc, output_nc, input_txt_nc, output_txt_nc, ngf=64,
        ...
        model_gen1 = [nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0), norm_layer(ngf),
                      activation]

        ### downsample
        for i in range(n_downsampling):  # 0- conv2d => norminstance => activation
            mult = 2 ** i
            model_gen1 += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1),
                           norm_layer(ngf * mult * 2), activation]
        self.model_gen_img = nn.Sequential(*model_gen1)
        ...
        for i in range(n_downsampling):  # 0-3
            mult = 2 ** (n_downsampling - i)
            model_gen2 += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=2, padding=1,
                                              output_padding=1),
                           norm_layer(int(ngf * mult / 2)), activation]
        model_gen2 += [nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0), nn.Tanh()]
        self.model_gen = nn.Sequential(*model_gen2)
    ...
    def forward(self, input_img, input_txt):
        input_cat = torch.cat((input_img, input_txt), dim=1)
        input_gen1 = self.model_gen_img(input_cat)
        input_gen2 = self.model_gen(input_gen1)
        return input_gen2, input_txt
...
class MultiscaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, 
        ...
        for i in range(num_D):
            netD = NLayerDiscriminator(input_nc, ndf, n_layers, norm_layer, use_sigmoid, getIntermFeat) #包含model5风模型
            if getIntermFeat:  #True
                for j in range(n_layers+2): #0-4
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, getIntermFeat=False):
        ...
        sequence += [[nn.Conv2d(nf, 1, kernel_size=kw, stride=1, padding=padw)]] #512=>1
